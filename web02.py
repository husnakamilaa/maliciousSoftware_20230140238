...
app = Flask(__name__)
app.secret_key = 'sqlinjection' # Kunci rahasia aplikasi Flask
...
def init_data():
    with connect_db() as conn:
        ...
        cur.executemany(
            ...
            [('alice','alicepw'), ('bob','bobpw'), ('user1', '123456')] # user1 ditambahkan
        )
        cur.executemany(
...
def search():
    ...
    query = f"SELECT id, user_id, content FROM time_line WHERE content LIKE '%{keyword}%'" # Kembali ke versi vulnerable
...
def index():
    if 'uid' in session:
        tl = get_time_lines()
        # Mendefinisikan pesan yang akan ditampilkan di konsol
        console_message = "YOU HAVE BEEN INFECTED HAHAHA!!!" 
...
<head>
  ...
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.4.0/dist/css/bootstrap.min.css" rel="stylesheet">
  {# Bagian ini akan mencetak pesan ke konsol #}
  <script>
    // Pastikan variabel console_message ada dan tidak kosong sebelum logging
    {% if console_message %}
...
<body>
  ...
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.4.0/dist/js/bootstrap.bundle.min.js"></script>
        ...
        ''').render(user=session['username'], tl=tl, console_message=console_message)
